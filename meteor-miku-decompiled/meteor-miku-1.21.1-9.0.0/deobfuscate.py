...
下載 Yarn mappings 並批量替換所有 Java 文件中的混淆類名、方法名、字段名
"""
...
def parse_tiny_mappings(jar_content):
    """解析 Tiny v2 格式映射（使用縮排層級判斷，正確處理 method/field）"""
    mappings = {
        'classes': {},
        'methods': {},
        'fields': {}
    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith('\t\t'):
                ...
                if kind == 'm' and current_class and len(parts) >= 4:
                    obf_method = parts[2]
                    named_method = parts[3]
                    # 只保留實際有命名的（排除 intermediary == named 的未映射項）
                    if obf_method.startswith('method_') and named_method != obf_method:
                        mappings['methods'][obf_method] = named_method

                elif kind == 'f' and current_class and len(parts) >= 4:
                    obf_field = parts[2]
                    named_field = parts[3]
                    # 只保留實際有命名的（排除 intermediary == named 的未映射項）
                    if obf_field.startswith('field_') and named_field != obf_field:
                        mappings['fields'][obf_field] = named_field
            else:
                # 頂層：類映射
                parts = line.strip().split('\t')
...
def save_mappings_to_file(mappings, filename='mappings_1.21.1.txt'):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write("\n## Methods\n")
        for obf, named in sorted(mappings['methods'].items()):
            f.write(f"{obf} -> {named}\n")

        f.write("\n## Fields\n")
        for obf, named in sorted(mappings['fields'].items()):
            f.write(f"{obf} -> {named}\n")
    
    print(f"✓ 映射已保存到 {filename}")

def replace_in_file(filepath, class_mappings, method_mappings, field_mappings):
    """替換單個文件中的混淆類名、方法名、字段名"""
    try:
    ...
    # 合併所有映射，按名稱長度降序排序（避免部分匹配問題）
    all_mappings = {**class_mappings, **method_mappings, **field_mappings}
...
def process_directory(source_dir, mappings):
    """批量處理目錄中的所有 Java 文件"""
    print(f"\n開始處理目錄: {source_dir}")
...
def main():
    ...
    # 4. 顯示示例映射
    print("\n示例映射 (Classes):")
    for i, (obf, named) in enumerate(mappings['classes'].items()):
        if i >= 5:
            break
        print(f"  {obf} -> {named}")

    print("\n示例映射 (Methods):")
    for i, (obf, named) in enumerate(mappings['methods'].items()):
        if i >= 5:
            break
        print(f"  {obf} -> {named}")

    print("\n示例映射 (Fields):")
    for i, (obf, named) in enumerate(mappings['fields'].items()):
        if i >= 5:
            break
        print(f"  {obf} -> {named}")
    ...
    if user_input.lower() == 'y':
        process_directory(SOURCE_DIR, mappings)
    else:
        print("已取消。你可以:")
