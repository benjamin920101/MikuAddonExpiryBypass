...
extends UniformStructure<Fortress> {
    public static final VersionMap<RegionStructure.Config> CONFIGS = new VersionMap<RegionStructure.Config>().add(MCVersion.v1_8, new RegionStructure.Config(16, 8, -1)).add(MCVersion.v1_16, new RegionStructure.Config(30, 4, 30084232)).add(MCVersion.v1_16_1, new RegionStructure.Config(27, 4, 30084232));
    ...
    @Override
    public boolean canStart(RegionStructure.Data<Fortress> data, long structureSeed, ChunkRand rand) {
        if (this.getVersion().isOlderThan(MCVersion.v1_16)) {
            ...
            if (data.chunkX != (data.chunkX & 0xFFFFFFF0) + rand.nextInt(8) + 4) {
                return false;
            }
            return data.chunkZ == (data.chunkZ & 0xFFFFFFF0) + rand.nextInt(8) + 4;
        }
    ...
    public boolean canSpawn(int chunkX, int chunkZ, BiomeSource source) {
        ...
        return this.isValidBiome(this.getVersion().isOlderThan(MCVersion.v1_16) ? source.getBiome(x, 0, z) : source.getBiomeForNoiseGen(x, 0, z));
    }
