...
extends LootGenerator {
    ...
    public boolean existsIn(MCVersion version) {
        if (this.introducedVersion != null && version.isOlderThan(this.introducedVersion)) {
            return false;
        }
    ...
    public int getEffectiveWeight(int luck) {
        return Math.max(MathHelper.floor((float)this.weight + (float)this.quality * (float)luck), 0);
    }

    @SafeVarargs
    public final LootEntry apply(Function<MCVersion, LootFunction> ... lootFunctions) {
        super.apply(Arrays.asList(lootFunctions));
        return this;
    }

    @SafeVarargs
    public final LootEntry when(Function<MCVersion, LootCondition> ... lootConditions) {
